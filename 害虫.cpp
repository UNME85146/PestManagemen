...
害虫::害虫(std::string 名字, std::vector<std::string> 特征, int 年龄)
    : 名字(名字), 特征(特征), 年龄(年龄), 红温值(0), 管理员权限(false)
{
}
...
void 害虫::显示信息() const
{
    ...
    std::cout << std::endl;
    std::cout << "管理员权限: " << (管理员权限 ? "是" : "否") << std::endl;
    std::cout << "特征: ";
...
void 害虫::加入QQ群()
{
    std::cout << "【系统通知】" << 名字 << " 加入了QQ群！" << std::endl;
    建政();
    吹牛();
}

bool 害虫::获取管理员权限() const
{
    return 管理员权限;
}

void 害虫::设置管理员权限(bool 权限)
{
    管理员权限 = 权限;
    if (权限) {
        std::cout << "【权限变更】" << 名字 << " 获得了管理员权限！" << std::endl;
    }
    else {
        std::cout << "【权限变更】" << 名字 << " 失去了管理员权限！" << std::endl;
    }
}
...
void 害虫::行为()
{
    ...
    if (红温值 > 100) {
        ...
        if (管理员权限) {
            ...
            std::cout << "【提示】" << 名字 << " 没有管理员权限，无法执行禁言和踢人操作！" << std::endl;
        }
        return;
    }
